 ...
 *
 * The reason for this is so that the rest of the factors are collected, even if
 * earlier
 * factors failed.
 *
...
public class MfaAuthenticationHandler implements AuthenticationSuccessHandler, AuthenticationFailureHandler {
	...
	private void saveMfaAuthentication(HttpServletRequest request, HttpServletResponse response,
			Authentication authentication) throws IOException, ServletException {
		if (!urlSuccess.equals("/home")) {
			MfaAuthentication mfa = new MfaAuthentication(authentication);
			HolderSecurity.setData(mfa);
